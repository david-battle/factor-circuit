...
---|------------|-----|------|------|-------------------------------
 ...
 9 |        149 |  —  | 1158 |  ?   | ABC survey plateau only (no BLIF artifact, no windowed resynth yet)
```
...
- **N=9 (Aug 20):** ABC survey UB = 1158 (hard plateau from round 1, 600s,
  `python3 survey.py 9 600 0`). No LB attempted. `survey.py` does not
  persist the best BLIF, so there is no `factor9_*.blif` artifact yet.
  **Ready next step:** regenerate the N=9 BLIF, run the ABC pipeline with
  an explicit `write_blif` to create `factor9_opt_final.blif`, then run
  `window_opt.py` on it (expect ~400-500 gates if the usual 40-60%
  windowed reduction holds).
