...
def build_and_solve(k, solver_name="cd153"):
    ...
    result = {"sat": sat, "conflicts": conflicts, "vars": nvars,
              "gate_info": gate_info, "output_info": output_info, "sig": sig}
...
print(f"Known: k<=10 UNSAT, k<=90 SAT (ABC heuristic)\n")
...
while lo < hi:
    mid = (lo + hi) // 2
    print(f"Trying k={mid} (searching [{lo}, {hi}])...", flush=True)

    t0 = time.time()
    result = build_and_solve(mid)
    dt = time.time() - t0
    total_time += dt

    status = "SAT" if result["sat"] else "UNSAT"
    print(
        f"  k={mid}: {status}  conflicts={result['conflicts']:>8}  "
        f"vars={result['vars']:>5}  time={dt:7.1f}s  cumulative={total_time:8.1f}s"
    )

    if result["sat"]:
        hi = mid
        if lo == hi:
            print(f"\n*** OPTIMUM: {mid} AND gates ***")
...
if lo == hi:
    print(f"\nTrying final k={lo}...", flush=True)
    t0 = time.time()
    result = build_and_solve(lo)
    dt = time.time() - t0
    total_time += dt
    status = "SAT" if result["sat"] else "UNSAT"
    print(
        f"  k={lo}: {status}  conflicts={result['conflicts']:>8}  "
        f"vars={result['vars']:>5}  time={dt:7.1f}s  cumulative={total_time:8.1f}s"
    )
    if result["sat"]:
        print(f"\n*** OPTIMUM: {lo} AND gates ***")
        decode_and_verify(result)
    else:
        print(f"\nNo solution found (lo={lo} UNSAT, which means there's a bug)")
